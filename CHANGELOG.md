...
* Homepage
	...
	* Show a banner if 'today' is a Hypercoin 'Holiday' (see more below)
* Minor fixes for running against Hypercoin Core v2.0
...
* New [/holidays](./holidays), a curated list of Hypercoin 'Holidays'
...
* Random Hypercoin-related quote shown in footer on each page load
* New [/quotes](./quotes), curated list of Hypercoin-related quotes (each quote also having its own page like [this](`./quote/0`))
...
* Improvements to no-`txindex` support: now available for all versions of Hypercoin Core
...
* Support for pruned nodes and nodes with disabled `txindex`! (HUGE Thanks to [@shesek](https://github.com/shesek))
	* Note: Currently only Hypercoin Core versions 0.21+ are able to support this feature (a future improvement is planned to make it available to all versions)
...
* New minor misc peer data available in Hypercoin Core RPC v1.0+
...
* Homepage UI tweaks
	* Remove "Hypercoin Explorer" H1 (it's redundant)
